...
4. **Durability:** retry registration survives only while the modal retains the same File object. Refresh/close recovery of pending registration remains unfinished. Local ciphertext cache is best effort and capped at 12 MiB; an offline export is only useful if the user actually saves it.
5. **Preservation:** no independently verified replicas, scheduled remote checks, trusted public object-hash anchoring or permanent-storage guarantee on testnet. A stored adjacent content hash is not independent proof against wholesale object replacement.
